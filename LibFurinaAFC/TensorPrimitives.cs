...
public unsafe class TensorPrimitives
{
    ...
    /// </remarks>
    public const nuint NonTemporalByteThreshold = 256 * 1024;
    ...
    /// </typeparam>
    public static void InvokeSpanSpanIntoSpan<T, TBinaryOperator>(
        ReadOnlySpan<T> x, ReadOnlySpan<T> y, Span<T> destination)
        where T : unmanaged
        where TBinaryOperator : struct, IBinaryOperator<T>
    {
        /* MODIFIED:
         * Remove parameters checking, requiring callers to check input.
         */
    ...
    /// <summary>x ^ y</summary>
    public readonly struct XorOperator<T> : IBinaryOperator<T> where T : IBitwiseOperators<T, T, T>
    {
